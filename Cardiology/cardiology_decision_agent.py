...
class CardiologyDecisionAgent:
    ...
    def _choose_primary_plan(
        ...
        if condition_id == "stemi" or "stemi_reperfusion_priority" in rule_ids:
            return plan_by_id.get("stemi_reperfusion_pathway") or plan_by_id.get("acs_initial_evaluation") or scored_candidates[0]
        if condition_id in {"nste_acs", "chronic_coronary_syndrome"}:
            return plan_by_id.get("acs_initial_evaluation") or scored_candidates[0]
        if condition_id in {"acute_decompensated_heart_failure", "hfrEF", "hfpEF"}:
            return plan_by_id.get("acute_hf_decongestion") or plan_by_id.get("hfrEF_gdmt_buildout") or scored_candidates[0]
        if condition_id == "atrial_fibrillation":
            return plan_by_id.get("af_rate_rhythm_anticoag") or scored_candidates[0]
        if condition_id == "supraventricular_tachycardia":
            return plan_by_id.get("svt_termination_pathway") or scored_candidates[0]
        if condition_id in {"acute_aortic_dissection", "ascending_aortic_aneurysm"}:
            return plan_by_id.get("aortic_emergency_pathway") or scored_candidates[0]
        if condition_id in {"severe_aortic_stenosis", "severe_mitral_regurgitation"}:
            return plan_by_id.get("valve_intervention_pathway") or scored_candidates[0]
        if condition_id in {"multivessel_cad_for_revascularization", "post_cardiotomy_low_output_syndrome"}:
            return plan_by_id.get("cabg_perioperative_pathway") or scored_candidates[0]
