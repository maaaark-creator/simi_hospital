...
class KnowledgeRetriever:
    ...
    def retrieve(
        ...
        matched_rules = [
            ...
            if rule["id"] in graph_ranking.get("ranked_rule_ids", safety_rule_ids)
        ]
        matched_rules.sort(
            key=lambda item: graph_ranking.get("rule_scores", {}).get(item.get("id", ""), 0.0),
            reverse=True,
        )
        related_drugs = self._collect_related_drugs_from_ranked_ids(
            graph_ranking.get("ranked_drug_ids", [])
        )
        ...
        return {
            ...
            "candidate_plan_scores": graph_ranking.get("plan_scores", {}),
    ...
    def _load_graph_settings(self) -> dict[str, Any]:
        ...
        return {
            ...
            "max_candidate_plans": int(retrieval_cfg.get("max_candidate_plans", 6)),
            "max_related_drugs": int(retrieval_cfg.get("max_related_drugs", 10)),
            "plan_expand_threshold": float(retrieval_cfg.get("plan_expand_threshold", 0.45)),
            "rule_expand_threshold": float(retrieval_cfg.get("rule_expand_threshold", 0.3)),
            "drug_expand_threshold": float(retrieval_cfg.get("drug_expand_threshold", 0.3)),
            "seed_weights": {
                "cluster": float(retrieval_cfg.get("cluster_seed_weight", 0.9)),
                "condition": float(retrieval_cfg.get("condition_seed_weight", 1.0)),
                "template": float(retrieval_cfg.get("template_seed_weight", 0.9)),
                "emergency": float(retrieval_cfg.get("emergency_seed_weight", 0.95)),
                "plan": float(retrieval_cfg.get("plan_seed_weight", 1.0)),
                "rule": float(retrieval_cfg.get("rule_seed_weight", 0.9)),
                "contra": float(retrieval_cfg.get("contra_seed_weight", 0.85)),
                "comorbidity": float(retrieval_cfg.get("comorbidity_seed_weight", 0.8)),
            },
            "edge_weights": {
                "cluster_to_condition": float(edge_weights.get("cluster_to_condition", 0.85)),
                "template_to_plan": float(edge_weights.get("template_to_plan", 0.95)),
                "emergency_to_plan": float(edge_weights.get("emergency_to_plan", 0.95)),
                "plan_to_drug": float(edge_weights.get("plan_to_drug", 0.9)),
                "condition_to_drug": float(edge_weights.get("condition_to_drug", 0.85)),
                "contra_to_drug": float(edge_weights.get("contra_to_drug", 0.9)),
                "condition_to_plan": float(edge_weights.get("condition_to_plan", 0.85)),
                "comorbidity_to_rule": float(edge_weights.get("comorbidity_to_rule", 0.75)),
            },
            "condition_plan_edges": graph_cfg.get("condition_plan_edges", {}),
        }
    ...
    def _build_knowledge_graph(self) -> tuple[dict[str, list[tuple[str, float, str]]], dict[str, str]]:
        ...
        known_condition_ids = {item["id"] for item in self.kb.data.get("medical_conditions", [])}
        known_condition_ids.update({item["id"] for item in self.kb.data.get("surgical_conditions", [])})
        for cluster in self.kb.data.get("chief_complaint_clusters", []):
            cluster_id = str(cluster.get("id") or "")
            for condition_id in cluster.get("high_priority_differentials", []):
                if condition_id in known_condition_ids:
                    add_edge(
                        ...
                        "condition",
                        str(condition_id),
                        edge_weights["cluster_to_condition"],
                        "cluster_has_differential",
                    )

        for template in self.kb.data.get("procedure_templates", []):
            template_id = str(template.get("id") or "")
            for plan_id in template.get("common_candidate_plans", []):
                add_edge("template", template_id, "plan", str(plan_id), edge_weights["template_to_plan"], "template_recommends_plan")

        for algorithm in self.kb.data.get("emergency_algorithms", []):
            algorithm_id = str(algorithm.get("id") or "")
            for plan_id in algorithm.get("common_candidate_plans", []):
                add_edge("emergency", algorithm_id, "plan", str(plan_id), edge_weights["emergency_to_plan"], "emergency_recommends_plan")

        for plan in self.kb.data.get("care_plans", []):
            plan_id = str(plan.get("id") or "")
            for ids in plan.get("candidate_drugs", {}).values():
                for drug_id in ids:
                    add_edge("plan", plan_id, "drug", str(drug_id), edge_weights["plan_to_drug"], "plan_uses_drug")

        for condition in self.kb.data.get("medical_conditions", []):
            condition_id = str(condition.get("id") or "")
            for drug_id in condition.get("common_related_drugs", []):
                add_edge("condition", condition_id, "drug", str(drug_id), edge_weights["condition_to_drug"], "condition_related_drug")

        for condition_id, plan_ids in self._graph_settings.get("condition_plan_edges", {}).items():
            for plan_id in plan_ids:
                add_edge("condition", str(condition_id), "plan", str(plan_id), edge_weights["condition_to_plan"], "condition_recommends_plan")
        ...
        for contra in self.kb.data.get("contraindication_catalog", []):
            ...
            for drug_id in contra.get("related_drugs", []):
                add_edge("contra", contra_id, "drug", str(drug_id), edge_weights["contra_to_drug"], "contra_related_drug")
        ...
        for comorbidity_rule in self.kb.data.get("comorbidity_rules", []):
            ...
            for safety_rule in safety_rules:
                ...
                safety_text = " ".join(
                    [
                        " ".join(safety_rule.get("trigger_conditions", [])),
                        str(safety_rule.get("name_zh") or ""),
                        str(safety_rule.get("action") or ""),
                    ]
                ).lower()
                if any(term and term in safety_text for term in trigger_terms):
                    add_edge("comorbidity", comorbidity_id, "rule", rule_id, edge_weights["comorbidity_to_rule"], "comorbidity_related_rule")
    ...
    def _graph_rank_entities(
        ...
        for item in matched_clusters:
            ...
            if item_id:
                seed_nodes[f"cluster:{item_id}"] = max(seed_nodes.get(f"cluster:{item_id}", 0.0), seed_weights["cluster"])
        for item in matched_medical_conditions + matched_surgical_conditions:
            item_id = str(item.get("id") or "")
            if item_id:
                seed_nodes[f"condition:{item_id}"] = max(seed_nodes.get(f"condition:{item_id}", 0.0), seed_weights["condition"])
        for item in matched_templates:
            item_id = str(item.get("id") or "")
            if item_id:
                seed_nodes[f"template:{item_id}"] = max(seed_nodes.get(f"template:{item_id}", 0.0), seed_weights["template"])
        for item in matched_emergency_algorithms:
            item_id = str(item.get("id") or "")
            if item_id:
                seed_nodes[f"emergency:{item_id}"] = max(seed_nodes.get(f"emergency:{item_id}", 0.0), seed_weights["emergency"])
        for item_id in candidate_plan_ids:
            if item_id:
                seed_nodes[f"plan:{item_id}"] = max(seed_nodes.get(f"plan:{item_id}", 0.0), seed_weights["plan"])
        for item_id in safety_rule_ids:
            if item_id:
                seed_nodes[f"rule:{item_id}"] = max(seed_nodes.get(f"rule:{item_id}", 0.0), seed_weights["rule"])
        for item in matched_contraindications:
            item_id = str(item.get("id") or "")
            if item_id:
                seed_nodes[f"contra:{item_id}"] = max(seed_nodes.get(f"contra:{item_id}", 0.0), seed_weights["contra"])
        for item in matched_comorbidity_rules:
            item_id = str(item.get("id") or "")
            if item_id:
                seed_nodes[f"comorbidity:{item_id}"] = max(seed_nodes.get(f"comorbidity:{item_id}", 0.0), seed_weights["comorbidity"])
        ...
        return {
            "ranked_plan_ids": self._rank_ids_by_score(plan_scores, self._graph_settings["plan_expand_threshold"], self._graph_settings["max_candidate_plans"], candidate_plan_ids),
            "ranked_rule_ids": self._rank_ids_by_score(rule_scores, self._graph_settings["rule_expand_threshold"], max(len(safety_rule_ids), 10), safety_rule_ids),
            "ranked_drug_ids": self._rank_ids_by_score(drug_scores, self._graph_settings["drug_expand_threshold"], self._graph_settings["max_related_drugs"], []),
            "plan_scores": plan_scores,
            "rule_scores": rule_scores,
            "graph_context": {
                "enabled": True,
                "seed_nodes": sorted(seed_nodes.keys()),
                "top_plan_ids": self._rank_ids_by_score(plan_scores, self._graph_settings["plan_expand_threshold"], self._graph_settings["max_candidate_plans"], candidate_plan_ids),
                "top_rule_ids": self._rank_ids_by_score(rule_scores, self._graph_settings["rule_expand_threshold"], max(len(safety_rule_ids), 10), safety_rule_ids),
                "top_drug_ids": self._rank_ids_by_score(drug_scores, self._graph_settings["drug_expand_threshold"], self._graph_settings["max_related_drugs"], []),
            },
        }
    ...
    def _rank_ids_by_score(
        ...
        ranked = [item_id for item_id, score in sorted(ids.items(), key=lambda item: item[1], reverse=True) if score >= min_threshold]
