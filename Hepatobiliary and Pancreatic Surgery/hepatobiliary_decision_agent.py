...
from hbp_decision_agent import HepatobiliaryDecisionAgent as _GraphHbpDecisionAgent


class HepatobiliaryDecisionAgent(_GraphHbpDecisionAgent):
    """Backward-compatible alias that uses the graph-enabled HBP decision logic."""


class _LegacyHepatobiliaryDecisionAgent:
    ...
    def decide_with_api(
        ...
        prompt = (
            "You are a hepatobiliary-pancreatic surgery decision-support agent.\n"
    ...
    def _choose_primary_condition(
        ...
        if "胰腺炎" in combined_text or "pancreatitis" in combined_text:
            return medical_by_id.get("acute_pancreatitis")
        if "胆漏" in combined_text or "bile leak" in combined_text:
            return surgical_by_id.get("postoperative_bile_leak")
        if "胰瘘" in combined_text or "pancreatic fistula" in combined_text:
            return surgical_by_id.get("postoperative_pancreatic_fistula")
    ...
    def _choose_primary_plan(
        ...
        if condition_id == "acute_cholangitis" or "source_control_needed_for_severe_cholangitis" in rule_ids:
            return plan_by_id.get("acute_cholangitis_resuscitation") or candidate_plans[0]
        if condition_id == "acute_pancreatitis":
            return plan_by_id.get("acute_pancreatitis_supportive_care") or candidate_plans[0]
        if condition_id == "obstructive_jaundice_needing_biliary_drainage":
            return plan_by_id.get("obstructive_jaundice_workup") or candidate_plans[0]
        if condition_id == "hcc_resection_candidate":
            return plan_by_id.get("hepatectomy_perioperative_pathway") or candidate_plans[0]
        if condition_id == "pancreatic_head_mass_resection_candidate":
            return plan_by_id.get("pancreaticoduodenectomy_pathway") or candidate_plans[0]
        if condition_id == "choledocholithiasis_for_ercp_or_surgery":
            return plan_by_id.get("choledocholithiasis_source_control") or candidate_plans[0]
        if condition_id == "postoperative_bile_leak":
            return plan_by_id.get("postoperative_bile_leak_management") or candidate_plans[0]
        if condition_id == "postoperative_pancreatic_fistula":
            return plan_by_id.get("postoperative_pancreatic_fistula_management") or candidate_plans[0]
        if condition_id == "upper_gi_or_hepatobiliary_bleeding":
            return plan_by_id.get("hepatobiliary_bleeding_stabilization") or candidate_plans[0]

        if "ercp" in procedure_text:
            return plan_by_id.get("choledocholithiasis_source_control") or candidate_plans[0]
        if "whipple" in procedure_text or "胰十二指肠切除" in procedure_text:
            return plan_by_id.get("pancreaticoduodenectomy_pathway") or candidate_plans[0]
        if "发热" in chief_text and "黄疸" in chief_text:
            return plan_by_id.get("acute_cholangitis_resuscitation") or candidate_plans[0]
    ...
    def _build_risk_flags(
        ...
        if primary_condition and primary_condition["id"] in {"acute_cholangitis", "upper_gi_or_hepatobiliary_bleeding", "postoperative_pancreatic_fistula"}:
            flags.append("当前疑似高危肝胆胰急症，应优先处理复苏与源控制")
