...
class KnowledgeRetriever:
    ...
    def retrieve(
        ...
        candidate_plans = [
            ...
            for plan_id in graph_ranking.get("ranked_plan_ids", candidate_plan_ids)
    ...
    def _load_graph_settings(self) -> dict[str, Any]:
        ...
        return {
            ...
            "max_candidate_plans": int(retrieval_cfg.get("max_candidate_plans", 7)),
            "max_related_drugs": int(retrieval_cfg.get("max_related_drugs", 10)),
            "plan_expand_threshold": float(retrieval_cfg.get("plan_expand_threshold", 0.42)),
        ...
        for cluster in self.kb.data.get("chief_complaint_clusters", []):
            ...
            for condition_id in cluster.get("high_priority_differentials", []):
                if condition_id in known_condition_ids:
                    add_edge("cluster", cluster_id, "condition", str(condition_id), edge_weights["cluster_to_condition"], "cluster_has_differential")
            ...
            for safety_rule in safety_rules:
                ...
                safety_text = " ".join(
                    [
                        ...
                        str(safety_rule.get("description") or ""),
                    ]
                ).lower()
    ...
    def _graph_rank_entities(
        ...
        top_rule_ids = self._rank_ids_by_score(rule_scores, self._graph_settings["rule_expand_threshold"], max(len(safety_rule_ids), 12), safety_rule_ids)
        ...
        return {
            ...
            "ranked_rule_ids": top_rule_ids,
            "ranked_drug_ids": top_drug_ids,
            "plan_scores": plan_scores,
            "rule_scores": rule_scores,
            "graph_context": {
                "enabled": True,
                "seed_nodes": sorted(seed_nodes.keys()),
                "top_plan_ids": top_plan_ids,
                "top_rule_ids": top_rule_ids,
                "top_drug_ids": top_drug_ids,
            },
        }
